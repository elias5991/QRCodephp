...
class Account
{
    ...
    public static function getAllUsers($mysqli)
    {
        ...
        foreach ($result as $r) {
            array_push($values, new Account(
                $r['user_id'], $r['email'], $r['type'],null, $r['password'],
                $r['firstName'], $r['lastName'], $r['role'],null, $r['phone_number']
            ));
        }
        return $values;
    }
    ...
    {
        $query = "select * from account where email ='". $this->email ."' and type_id ='". $this->typeID ."' and firstName='" . $this->firstName . "' and lastName ='". $this->lastName."'and role_id='" . $this->roleID ."'and phone_number='" . $this->phoneNumber . "';";
        $result = mysqli_query($mysqli, $query);
    ...
    {
        $query = "select * from account where user_id =".$this->id .";";
        $result = mysqli_query($mysqli, $query);
        if (mysqli_num_rows($result) < 1) return false;
        return true;
    }
    ...
    {
        $query = "update account  set email ='". $this->email ."', type_id ='". $this->typeID ."', firstName='" . $this->firstName . "',lastName ='". $this->lastName."', role_id='" . $this->roleID ."', phone_number='" . $this->phoneNumber . "' where user_id=".$this->id.";";
        if (mysqli_query($mysqli, $query)) return true;
        return false;
    }
    ...
    {
        $query = "select * from account where user_id =" .$this->id ." and email ='". $this->email ."'and type_id ='". $this->typeID ."'and firstName='" . $this->firstName . "'and lastName ='". $this->lastName."'and role_id='" . $this->roleID ."'and phone_number='" . $this->phoneNumber . "';";
        $result = mysqli_query($mysqli, $query);
        if (mysqli_num_rows($result) < 1) return false;
        return true;
    }
